...
实现进度：阶段 A 的首个纵向版本已经提供模块化手动编辑、自动保存、实时结构校验和内嵌试玩；字段级 LLM 灵感、补全、润色、一致性检查及审阅账本也已接入。阶段 B 已落地：M1（决定 11 的不可变版本快照发布模型：drafts/releases 目录分层、发布门槛、版本递增、回滚指针、故事发现优先已发布快照）、M2（创作/游玩模型分开配置）、M3（短简报 → 概念提案：规模档位模板、无状态提案端点、概念以未审阅状态写入蓝图）。完整故事分阶段生成（8.2 节管线）与审阅中心仍按后续阶段实现。
...
9. 产品最终形态是用户在网页端登录使用的托管平台（参照 isekaizero.ai），不存在向平台外分发内容的场景。后续会提供多种模型可选择，创作模型和游玩模型分开配置；当前开发阶段沿用 kimi 和 deepseek。
10. 新故事规模按"代码定预算区间、概念阶段由 LLM 提出具体配置并说明理由、创作者拍板"三层机制确定。规模区间与 `target_duration_minutes` 挂钩，偏离区间只触发创作质量提示，不是阻塞校验错误。
11. "发布"采用不可变版本快照模型：可变草稿与不可变发布快照是两个实体；每次发布产生新快照与发布记录（版本号、源草稿 revision、审阅戳）；回滚是把当前版本指针指回旧快照；游玩会话绑定具体快照版本。不设计 zip 内容包、校验和分发等平台外分发机制。
...
- 姓名与公开形象（即玩家人物卡的公开字段，进入玩家感知与人物列表；2026-08-09 起与原"公开身份"合并为一个字段）；
- 私人目标；
- 玩家表达和能力边界；
- 初始场景。
...
1. 第一版是仅本地单用户使用，还是从一开始就需要远程账户和云端保存？——部分已定（见决定 9）：最终形态是托管平台，但开发阶段保持本地单用户；领域模型（草稿/快照/发布记录/版本指针）按可平移到云端存储设计。
2. ~~“发布”在第一阶段是生成仓库内正式内容文件，还是还需要独立的内容包和版本清单？~~ 已定，见决定 11：采用不可变版本快照模型。
3. ~~LLM provider 是否沿用当前 DeepSeek/Kimi 配置，还是工作室需要独立的生成模型选择？~~ 已定，见决定 9：创作/游玩模型分开配置，当前沿用 kimi 和 deepseek。
4. 第一版是否只支持纯文本字段，还是开场和简介需要基础富文本？
5. ~~新故事的默认规模模板如何定义，例如短篇、中篇、长篇分别建议多少人物、场景和模块？~~ 已定，见决定 10：代码定区间、概念阶段商讨、创作者拍板；初始数值从现有故事归纳。
