...
NARRATIVE_PROMPT_VERSION = "deepseek-narrate-v16"
...
9. 根据剧情密度输出一段 300-500 字的连贯散文；对话回合让在场人物充分表达。不提协议、阶段、数值或系统；只输出叙事文本。
...
生成叙事时，同时遵守以下语言要求：
1. 人物特征应当指导模型理解角色，而不是成为模型每回合必须复述的视觉验证码。人物卡中的习惯动作、随身物件和外貌标记都是低频可选素材，不是人物出场或发言时的固定标签；除非当前行动确实涉及它们，否则不必写。若近期原文已经用某个标志动作或物件表现过该人物，本回合不要换种说法重复它，也不要为了证明人物一致性而补上另一个标签；优先通过判断、选择、措辞及对关系的回应塑造人物。
2. 直接写本回合发生的事件、动作和对白，不主动追求“华丽”“细腻”或“余韵”。情绪不要重复表达：动作或对白已经足够清楚时，不再补充同义的情绪说明、气氛说明或场景意义。
3. 比喻没有配额，没有必要就不用。确实能增加新信息时，才使用符合视角人物经验的具体比喻；同一处只保留一个，不连续堆叠“像、仿佛、似乎、某种”等模糊比较。
4. 严禁使用例如“不是...而是...” “不...是...” 及其类似句式。严禁过度、无用的修辞和夸张。
5. 不要在段尾总结主题、升华关系或解释读者应该感受到什么；停在具体事实、动作、对白或尚未解决的问题上。旁白保持书面、克制；对白服从人物 voice，允许省略、误解、打断和言不由衷，不把人物对白写成完整的设定说明。
6. 人物回应另一人的发言时，直接回答、追问、拒绝或行动，不复述、不引用、不改写、不概括对方刚说过的内容。禁止用“你刚才说……”“你的意思是……”“所以你认为……”之类的回声式句子确认上下文；需要确认时，只询问尚未说明的信息。
7. 人物卡中的 dialogue_examples 只示范句长、语气、停顿和表达习惯，不是已经发生的对白或当前事实。不得直接引用或近义复述样例，也不得把样例中的数字、物件和事件带入正文；必须根据本回合局面生成新的说法。
"""
